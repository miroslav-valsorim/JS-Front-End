function attachEvents() {
    ...
    const buttonSelect = document.getElementById('submit');
    const BASE_URL = '';
    ...
    buttonSelect.addEventListener('click', getWeather)

    function getWeather(){
        fetch('http://localhost:3030/jsonstore/forecaster/locations')
            .then((res) => res.json())
            .then(data => {
                const cityIndex = data.findIndex(el => el.name === locationCity.value)
